...
ask_into() {
  ...
  answer="$(trim "$answer")"
  if [[ -z "$answer" ]]; then printf -v "$__var_name" '%s' "$default"; else printf -v "$__var_name" '%s' "$answer"; fi
}
...
ask_required_into() {
  ...
  while true; do
    ...
    IFS= read -r answer < /dev/tty || true
    answer="$(trim "$answer")"
    [[ -n "$answer" ]] && { printf -v "$__var_name" '%s' "$answer"; return 0; }
...
ask_yes_no() {
  ...
  case "${answer,,}" in y|yes) return 0 ;; n|no) return 1 ;; *) [[ "$default" == "Y" ]] ;; esac
}
...
ensure_docker() {
  if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then ok "Docker уже установлен"; return 0; fi
...
secret = sys.argv[1].encode(); role = sys.argv[2]
...
check_required_assets() {
  ...
  for f in "${files[@]}"; do [[ -f "$f" ]] || { warn "Не найден файл: $f"; missing=1; }; done
  [[ $missing -eq 0 ]] || err "Не хватает файлов в volumes/"
}
...
main() {
  require_root; ensure_requirements
  clear || true
  cat <<'EOB'
╔══════════════════════════════════════════════╗
...
EOB
  ...
  case "$MODE" in
    1) EXTERNAL_MODE="domain"; ask_required_into EXTERNAL_HOST "Домен Supabase" ;;
    2) EXTERNAL_MODE="ip"; ask_required_into EXTERNAL_HOST "IP или hostname сервера" ;;
  ...
  if ask_yes_no "Разрешить регистрацию новых пользователей?" "Y"; then DISABLE_SIGNUP="false"; else DISABLE_SIGNUP="true"; fi
  if [[ "$EXTERNAL_MODE" == "domain" ]]; then SERVICE_URL_SUPABASEKONG="https://$EXTERNAL_HOST"; else SERVICE_URL_SUPABASEKONG="http://$EXTERNAL_HOST:$KONG_HTTP_PORT"; fi
