...
ask(){ local p="$1" d="${2:-}" a; if [[ -n "$d" ]]; then read -r -p "$p [$d]: " a || true; a="$(trim "$a")"; printf '%s' "${a:-$d}"; else read -r -p "$p: " a || true; printf '%s' "$(trim "$a")"; fi; }
ask_secret(){ local p="$1" d="${2:-}" a; read -r -s -p "$p [$d]: " a || true; echo; a="$(trim "$a")"; printf '%s' "${a:-$d}"; }
ask_yes_no(){ local p="$1" d="${2:-Y}" a shown; [[ "$d" == "Y" ]] && shown='[Y/n]' || shown='[y/N]'; read -r -p "$p $shown: " a || true; a="$(trim "$a")"; a="${a:-$d}"; case "${a,,}" in y|yes) return 0;; n|no) return 1;; *) [[ "$d" == "Y" ]];; esac; }
random_hex(){ openssl rand -hex 32; }
random_password(){ openssl rand -hex 16; }

ensure_requirements(){
  [[ -f "$COMPOSE_TEMPLATE" ]] || err "Не найден $COMPOSE_TEMPLATE"
  command -v python3 >/dev/null 2>&1 || apt-get update && apt-get install -y python3 >/dev/null 2>&1
}

ensure_docker(){
  if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then ok "Docker уже установлен"; return; fi
...
BANNER
  ...
  case "$ACCESS_MODE" in
    1) EXTERNAL_MODE="domain" ; EXTERNAL_HOST="$(ask "Домен" "supabase.example.com")" ; SCHEME="https" ;;
    2) EXTERNAL_MODE="ip" ; EXTERNAL_HOST="$(ask "IP или hostname сервера" "127.0.0.1")" ; SCHEME="http" ;;
    *) err "Неверный режим" ;;
  esac

  INSTALL_DIR="$(ask "Папка установки" "/opt/supabase")"
  POSTGRES_DB="$(ask "PostgreSQL database" "postgres")"
  DB_PUBLIC_PORT="$(ask "Внешний порт Postgres" "6543")"
  ...
  DISABLE_SIGNUP="$(ask "Disable signup (true/false)" "false")"
  ENABLE_EMAIL_SIGNUP="$(ask "Enable email signup (true/false)" "true")"
  ENABLE_ANONYMOUS_USERS="$(ask "Enable anonymous users (true/false)" "false")"
  ENABLE_EMAIL_AUTOCONFIRM="$(ask "Enable email autoconfirm (true/false)" "false")"
  ENABLE_PHONE_SIGNUP="$(ask "Enable phone signup (true/false)" "true")"
  ENABLE_PHONE_AUTOCONFIRM="$(ask "Enable phone autoconfirm (true/false)" "true")"
  ...
  echo "  api url:       $SERVICE_URL_SUPABASEKONG"
  echo "  studio url:    http://$EXTERNAL_HOST:$STUDIO_PORT"
  echo "  db port:       $DB_PUBLIC_PORT"
  echo

  ask_yes_no "Сохранить .env и продолжить?" "Y" || err "Отменено"
  write_env
  validate_env
  verify_required_templates
  ensure_docker
